...
router.post("/", async (req, res) => {
    ...
    if (req.user.isStudent()) {
        return res.status(400).send("只有講師帳號能發佈新課程。。。");
    }
    ...
    try {
        ...
        return res.status(400).send("無法創建課程。。。");
    }
});
...
router.patch("/:_id", async (req, res) => {
    ...
    try {
        ...
        if (!courseFound) {
            return res.status(400).send("找不到課程內容，無法更新課程內容");
        }
        ...
        if (courseFound.instructor.equals(req.user._id)) {
            ...
            return res.send({ message: "成功更新課程", updatedCourse });
        } else {
